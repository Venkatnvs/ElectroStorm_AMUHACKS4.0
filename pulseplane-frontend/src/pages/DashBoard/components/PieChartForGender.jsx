...
const PieChartForGender = ({ chartData }) => {
  const cropTypes = [...new Set(chartData.map(item => item.gender))];
  ...
  let newChartData = chartData.map(item => {
    return {
      ...
      fill: `var(--color-${item.gender.toLowerCase().replace(' ', '-')})`,
    };
  });
  ...
  return (
    <Card className='flex flex-col'>
      <CardHeader className='items-center pb-0'>
        <CardTitle>Gender based Employee</CardTitle>
        <CardDescription>
          A breakdown of the fields by gender
        </CardDescription>
      </CardHeader>
...
export default PieChartForGender;
